...
class KixflagshipSink extends RequestSink {
  ...
  KixflagshipSink(ApplicationConfiguration appConfig) : super(appConfig) {
    logger.onRecord.listen((rec) => print("$rec ${rec.error ?? ""} ${rec.stackTrace ?? ""}"));

    var options = new KixflagshipConfiguration(appConfig.configurationFilePath);
  ...
  void setupRouter(Router router) {
    router.route("/chat_room")
      .generate( () => new ChatRoomController() );
  }
...
class KixflagshipConfiguration extends ConfigurationItem {
  KixflagshipConfiguration(String fileName) : super.fromFile(fileName);
